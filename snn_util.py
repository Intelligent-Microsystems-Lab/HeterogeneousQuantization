...
def spike_nonlinearity_bwd(ctx, g):
    u, thr = ctx
    return (
        g * vmap(grad(jax.nn.sigmoid))(u - thr),
        None,
    )
...
def vr_loss(alpha_vr, pred, target):
    ...
    c_pred = vmap(convt_run, (None, 0, None))(
        alpha_vr, pred, jnp.zeros(so_size)
    )
    c_target = vmap(convt_run, (None, 0, None))(
        alpha_vr, target, jnp.zeros(so_size)
    )
...
def update_w(
    ...
    loss, gwb = value_and_grad(loss_pred, argnums=(0, 1))(
        get_params(opt_state)[0],
        get_params(opt_state)[1],
        alpha,
        gamma,
        alpha_vr,
        thr,
