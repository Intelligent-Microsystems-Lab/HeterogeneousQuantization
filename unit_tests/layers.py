...
class ConvLayer(object):
  def __init__(
    ...
    self.output_size = (
        math.floor(
            (self.input_size + (2 * self.padding) - self.kernel_size)
            / self.stride
        )
        + 1
    )
  ...
  def update_weights(self, e, update_weights=False, sign_reverse=False):
    ...
    if update_weights:
      if (
          sign_reverse == True
      ):  # This is necessary because PC and backprop learn gradients with different signs grad_pc = -grad_bp
        self.kernel -= self.learning_rate * torch.clamp(
            dW * 2, -50, 50
        )
      else:
        self.kernel += self.learning_rate * torch.clamp(
  ...
  def save_layer(self, logdir, i):
    np.save(
        logdir + "layer_" + str(i) + "_weights.npy",
        self.kernel.detach().cpu().numpy(),
    )
...
class AvgPool(object):
  ...
  def backward(self, y):
    ...
    return F.interpolate(
        y, scale_factor=(1, 1, self.kernel_size, self.kernel_size)
    )
...
class ProjectionLayer(object):
  def __init__(
      self, input_size, output_size, f, df, learning_rate, device="cpu"
  ):
    self.input_size = input_size
    ...
    self.weights = (
        torch.empty((self.Hid, self.output_size))
  ...
  def update_weights(self, e, update_weights=False, sign_reverse=False):
    ...
    if update_weights:
      if sign_reverse == True:
        self.weights -= self.learning_rate * torch.clamp(
            (dw * 2), -50, 50
        )
      else:
        self.weights += self.learning_rate * torch.clamp(
...
class FCLayer(object):
  def __init__(
    ...
    self.device = device
    self.weights = (
        torch.empty([self.input_size, self.output_size])
        .normal_(mean=0.0, std=0.05)
        .to(self.device)
    )
