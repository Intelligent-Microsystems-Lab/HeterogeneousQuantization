...
class UnitTests(absltest.TestCase):
  ...
  def test_pc_conv_out_equality(self):
    ...
    class test_pc_nn(PC_NN):
      def setup(self):
        self.layers = [
            ConvolutionalPC(features=6, kernel_size=(5, 5), padding='VALID', config=cfg),
            ConvolutionalPC(features=12, kernel_size=(5, 5), padding='VALID', config=cfg),
        ]
    ...
    np.testing.assert_almost_equal(np.array(out), np.array(jnp.moveaxis(out_ref, (0,1,2,3) , (0,3,2,1))), decimal=6)
  ...
  def test_pc_conv_err_equality(self):
    ...
    np.testing.assert_almost_equal(np.array(err["layers_0"]), np.array(jnp.moveaxis(err0_ref, (0,1,2,3) , (0,3,2,1))), decimal=6)
