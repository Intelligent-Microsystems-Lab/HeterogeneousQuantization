...
def rnn_accuracy(model, target_batch):
    ...
    for i in range(len(model.y_preds)):  # this loop is over the seq_len
        for b in range(B):
            if torch.argmax(target_batch[i, :, b]) == torch.argmax(
                model.y_preds[i][:, b]
            ):
                accuracy += 1
    return accuracy / (L * B)
...
class PC_RNN(object):
    def __init__(
        ...
        self.Wh = set_tensor(
            torch.from_numpy(
                np.random.normal(0, 0.05, [self.hidden_size, self.hidden_size])
            )
        )
        self.Wx = set_tensor(
            torch.from_numpy(
                np.random.normal(0, 0.05, [self.hidden_size, self.input_size])
            )
        )
        self.Wy = set_tensor(
            torch.from_numpy(
                np.random.normal(0, 0.05, [self.output_size, self.hidden_size])
            )
        )
        self.h0 = set_tensor(
            torch.from_numpy(
                np.random.normal(0, 0.05, [self.hidden_size, self.batch_size])
            )
        )
    ...
    def forward_sweep(self, input_seq):
        ...
        for i, inp in enumerate(input_seq):
            self.h_preds[i + 1] = self.fn(
                self.Wh @ self.h_preds[i] + self.Wx @ inp
            )
        ...
        with torch.no_grad():
            ...
            for i, (inp, targ) in reversed(
                ...
                for n in range(self.n_inference_steps):
                    ...
                    hdelta -= self.Wy.T @ (
                        self.e_ys[i]
                        * linear_deriv(self.Wy @ self.h_preds[i + 1])
                    )
                    if i < len(target_seq) - 1:
                        fn_deriv = self.fn_deriv(
                            self.Wh @ self.h_preds[i + 1]
                            + self.Wx @ input_seq[i + 1]
                        )
    ...
    def update_weights(self, input_seq, update_weights=True):
        with torch.no_grad():
            ...
            for i in reversed(list(range(len(input_seq)))):
                fn_deriv = self.fn_deriv(
                    self.Wh @ self.h_preds[i] + (self.Wx @ input_seq[i])
                )
                dWy += (
            ...
            if update_weights:
                self.Wy += self.weight_learning_rate * torch.clamp(
                    dWy, -self.clamp_val, self.clamp_val
                )
                self.Wx += self.weight_learning_rate * torch.clamp(
                    dWx, -self.clamp_val, self.clamp_val
                )
                self.Wh += self.weight_learning_rate * torch.clamp(
                    dWh, -self.clamp_val, self.clamp_val
                )
            return dWy, dWx, dWh
        ...
        with torch.no_grad():
            ...
            for n in range(n_epochs):
                ...
                for i, (inp, target) in enumerate(dataset):
                    ...
                    input_seq = set_tensor(
                        torch.from_numpy(
                            onehot(inp.transpose(), self.vocab_size)
                        )
                    )
                    target_seq = set_tensor(
                        torch.from_numpy(
                            onehot(target.transpose(), self.vocab_size)
                        )
                    )

                    ###
                    # create inital weights which can be reproduced.
                    ###
                    self.Wh = self.Wh.t()
                    self.Wx = self.Wx.t()
                    self.Wy = self.Wy.t()
                    ...
                    self.Wh = (
                        torch.reshape(
                            torch.arange(
                                torch.prod(torch.tensor(self.Wh.shape))
                            ),
                            self.Wh.shape,
                        )
                        / torch.prod(torch.tensor(self.Wh.shape))
                    ...
                    self.Wx = (
                        torch.reshape(
                            torch.arange(
                                torch.prod(torch.tensor(self.Wx.shape))
                            ),
                            self.Wx.shape,
                        )
                        / torch.prod(torch.tensor(self.Wx.shape))
                    ...
                    self.Wy = (
                        torch.reshape(
                            torch.arange(
                                torch.prod(torch.tensor(self.Wy.shape))
                            ),
                            self.Wy.shape,
                        )
                        / torch.prod(torch.tensor(self.Wy.shape))
                    ...
                    self.h0 = torch.zeros_like(self.h0)
                    ...
                    np.save("pc_rnn_dWh.npy", dWh)
                    np.save("pc_rnn_dWx.npy", dWx)
                    np.save("pc_rnn_dWy.npy", dWy)
                    raise Exception(
                        "Done - created files with gradients after one batch"
                    )
...
class Backprop_RNN(object):
    ...
    def backward_sweep(self, input_seq, target_seq):
        ...
        for i, (inp, targ) in reversed(
            ...
            dhdh = self.Wy.T @ (
                self.dys[i] * linear_deriv(self.Wy @ self.hs[i + 1])
            )
            if i < len(target_seq) - 1:
                fn_deriv = self.fn_deriv(
                    self.Wh @ self.hs[i + 1] + self.Wx @ input_seq[i + 1]
                )
    ...
    def update_weights(self, input_seq, update_weights=True):
        ...
        for i, inp in reversed(list(enumerate(input_seq))):
            fn_deriv = self.fn_deriv(
                self.Wh @ self.hs[i] + self.Wx @ input_seq[i]
            )
            dWy += (
                self.dys[i] * linear_deriv(self.Wy @ self.hs[i + 1])
            ) @ self.hs[i + 1].T
        ...
        if update_weights:
            self.Wy += self.learning_rate * torch.clamp(
                dWy, -self.clamp_val, self.clamp_val
            )
            self.Wx += self.learning_rate * torch.clamp(
                dWx, -self.clamp_val, self.clamp_val
            )
            self.Wh += self.learning_rate * torch.clamp(
                dWh, -self.clamp_val, self.clamp_val
            )
        return dWy, dWx, dWh
            ...
            for n in range(n_epochs):
                ...
                for i, (inp, target) in enumerate(dataset):
                    input_seq = set_tensor(
                        torch.from_numpy(
                            onehot(
                                inp.reshape(seq_length, self.batch_size),
                                self.vocab_size,
                            )
                        )
                    )
                    target_seq = set_tensor(
                        torch.from_numpy(
                            onehot(
                                target.reshape(seq_length, self.batch_size),
                                self.vocab_size,
                            )
                        )
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_inference_steps", type=int, default=10)
