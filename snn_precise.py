...
from snn import snn_fc_net_run, vr_loss, state_init
from visualize import pattern_plot
...
with open("smile_data_set/input_700_250_25.pkl", 'rb') as f:
    x_train = jnp.array(pickle.load(f)).transpose()
with open("smile_data_set/smile95.pkl", 'rb') as f:
...
params = [{'w' : random.normal(subkey1, (hidden1_size, in_size))}, 
          {'w' : random.normal(subkey2, (hidden2_size, hidden1_size))},
          {'w' : random.normal(subkey3, (out_size, hidden2_size))}]
...
pred = snn_fc_net_run(params_fixed, params, x_train, state0)
pattern_plot(x_train.transpose(), pred, y_train)
