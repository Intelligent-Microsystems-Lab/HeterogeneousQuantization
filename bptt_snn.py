...
parser.add_argument(
    ...
    type=float,
    default=0.85,
    help="Time constant for Van Rossum distance",
)
...
for e in range(args.epochs):
    ...
    for x_train, y_train in train_dl:
        ...
        acc_ta = (
            acc_ta * s_ta
            + float(acc_compute(pred, y_train)) * int(x_train.shape[0])
    ...
    for x_test, y_test in test_dl:
        ...
        acc_te = (
            acc_te * s_te
            + float(acc_compute(pred, y_test)) * int(x_test.shape[0])
...
curve_plot(
    ...
    test_hist,
    model_uuid + "_curve",
    str(loss_hist[-1]) + " " + str(args.alpha_vr),
)
