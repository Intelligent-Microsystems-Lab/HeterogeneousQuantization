...
class TrainTest(absltest.TestCase):
  ...
  def test_enet_create_model(self):
    """Tests creating model."""
    config_module = importlib.import_module(
        'efficientnet.configs.efficientnet-lite0')
    config = config_module.get_config()
    model = train_utils.create_model(
        model_cls=models.EfficientNetB0,  # pylint: disable=protected-access
        num_classes=1000,
        config=config)
    params, quant_params, batch_stats, _, _, _ = train_utils.initialized(
  ...
  def test_enet_train_and_evaluate(self):
  ...
  def test_resnet_create_model(self):
    """Tests creating model."""
    config_module = importlib.import_module('resnet.configs.resnet18_fp32')
    config = config_module.get_config()
    model = train_utils.create_model(
        model_cls=models.ResNet1,
        num_classes=1000,
  ...
  def test_resnet_train_and_evaluate(self):
    """Tests training and evaluation loop using mocked data."""
    # Create a temporary directory where tensorboard metrics are written.
    workdir = tempfile.mkdtemp()
    data_dir = '../unit_tests/tensorflow_datasets'
