...
def plot_bits(config: ml_collections.ConfigDict, workdir: str):
  ...
  try:
    with open('/Users/clemens/Desktop/sqnxt_bits.pkl', 'rb') as f:
      enet_bits = pickle.load(f)
    with open('/Users/clemens/Desktop/sqnxt_max.pkl', 'rb') as f:
      max_data = pickle.load(f)
  except:
    enet_bits, max_data = load_data(config, workdir)
    with open('/Users/clemens/Desktop/sqnxt_bits.pkl', 'wb') as f:
      pickle.dump(enet_bits, f)
    with open('/Users/clemens/Desktop/sqnxt_max.pkl', 'wb') as f:
  ...
  def plot_fig_num(omit, name):
    ...
    if '/act' in omit:
      ...
      ax.plot([0, 227 + 1], [2 / 7, 2 / 7], color='grey', alpha=.5,
              linestyle='--', linewidth=3, zorder=0)
      ax.plot([0, 227 + 1], [4 / 7, 4 / 7], color='grey', alpha=.5,
              linestyle='--', linewidth=3, zorder=0)
      ax.plot([0, 227 + 1], [6 / 7, 6 / 7], color='grey', alpha=.5,
              linestyle='--', linewidth=3, zorder=0)

      ax.plot([0, 227 + 1], [-2 / 6, -2 / 6], color='grey', alpha=.5,
              linestyle='--', linewidth=3, zorder=0)
      ax.plot([0, 227 + 1], [-4 / 6, -4 / 6], color='grey', alpha=.5,
              linestyle='--', linewidth=3, zorder=0)
      ax.plot([0, 227 + 1], [-6 / 6, -6 / 6], color='grey', alpha=.5,
              linestyle='--', linewidth=3, zorder=0)
    elif '/weight' in omit:
      ...
      ax.plot([0, 227 + 1], [1 / 4.1, 1 / 4.1], color='grey', alpha=.5,
              linestyle='--', linewidth=3, zorder=0)
      ax.plot([0, 227 + 1], [3 / 4.1, 3 / 4.1], color='grey', alpha=.5,
              linestyle='--', linewidth=3, zorder=0)
    ...
    for k, v in enet_bits.items():
      ...
      if not skip:
        if v[3] == 65:
          label = 'Separable Conv.'
          color = 'g'
        elif ('stem_conv' in k):
          label = '7x7 Convolution'
          color = 'b'
    ...
    ax.text(-10, 0.3, '# Bits', dict(size=23), rotation=90)
    ax.text(-10, -.6, 'log10 #', dict(size=23), rotation=90)
    ax.annotate('', xy=(0, 0), xytext=(pos + 4, 0), arrowprops=dict(
    ...
    if '/act' in omit:
      ...
      ax.text(90, -1.2, "Weights SqueezeNext", dict(size=23))
    else:
      plt.ylim(-1.13, 1.1)
      ax.set_yticks([-6 / 6, -4 / 6, -2 / 6, 0., 2 / 5, 4 / 5, ])
      ax.set_yticklabels([6, 4, 2, 0, 2, 4])
      ax.text(90, -1.2, "Activations SqueezeNext", dict(size=23))
  ...
  def plot_fig_aw(name):
    ...
    extra_entry = False
    for k, v in enet_bits.items():
      if ('bias' not in k) and (k != '/stem_conv/act'):
        if v[3] == 65:
          label = 'Separable Conv.'
          color = 'g'
        elif ('stem_conv' in k):
    ...
    # ax.set_xlabel("#quantization", fontsize=font_size, fontweight='bold')
    # ax.set_ylabel("bits", fontsize=font_size, fontweight='bold')
    ax.text(-10, 0.3, 'W # Bits', dict(size=23), rotation=90)
    ax.text(-10, -.6, 'A # Bits', dict(size=23), rotation=90)
    ax.annotate('', xy=(0, 0), xytext=(227 + 2, 0), arrowprops=dict(
        arrowstyle='<-, head_width=.3, head_length=1.', lw=3), zorder=20)
    ...
    ax.text(90, -.82, "Bits SqueezeNext", dict(size=23))
  ...
  # plot_fig_num(['/bias'], 'SqueezeNext/figures/bitwidths_all.png')
  plot_fig_num(['/act', '/bias'], 'squeezenext/figures/bitwidths_w.png')
  plot_fig_num(['/weight', '/bias'], 'squeezenext/figures/bitwidths_a.png')
  plot_fig_aw('squeezenext/figures/bitwidths_aw.png')
