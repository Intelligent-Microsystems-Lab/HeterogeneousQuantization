...
def get_config():
  ...
  config.quant.invertedresidual.weight = partial(
      DuQ, bits = 8, init_fn=partial(gaussian_init, axis=(0, 1, 2)), round_fn=round_ewgs)
  ...
  config.quant.invertedresidual.act = partial(DuQ, bits = 8, act=True, init_fn=partial(
  ...
  #     percentile_init, perc=99.9), round_fn=round_invtanh, bitwidth_min=1, d_max=8)
  config.quant.head.act = partial(DuQ, bits = 8, act=True, init_fn=partial(
      percentile_init, perc=99.9), round_fn=round_invtanh)

  # Final linear layer.
  config.quant.dense = ml_collections.ConfigDict()
  config.quant.dense.weight = partial(
      DuQ, bits = 8, init_fn=gaussian_init, round_fn=round_ewgs)
  # config.quant.dense.act = partial(parametric_d_xmax, act=True, init_fn=partial(
  #     percentile_init, perc=99.9), round_fn=round_invtanh, bitwidth_min=1, d_max=8)
  config.quant.dense.act = partial(DuQ, bits = 8, act=True, init_fn=partial(
      percentile_init, perc=99.9), round_fn=round_invtanh)
