# Copied from https://github.com/deepmind/dnc/blob/master/dnc/repeat_copy.py
...
class RepeatCopy:  # snt.AbstractModule
    """Sequence data generator for the task of repeating a random binary
    pattern. When called, an instance of this class will return a tuple of
    tensorflow ops (obs, targ, mask), representing an input sequence, target
    sequence, and binary mask. Each of these ops produces tensors whose first
    two dimensions represent sequence position and batch index respectively.
    The value in mask[t, b] is equal to 1 iff a prediction about targ[t, b, :]
    should be penalized and 0 otherwise.
    ...
    def __init__(
        ...
        Args:
          ...
          time_average_cost: If true, the cost at each time step will be
              divided by the `true`, sequence length, the number of non-masked
              time steps, in each sequence before any subsequent reduction over
              the time and batch dimensions.
        """
        super(RepeatCopy, self).__init__()
    ...
    def _build(self, T):
        ...
        max_length_batch = jnp.max(total_length_batch)
        ...
        residual_mask_pad = [
            jnp.zeros([residual_length_batch[i]]) for i in range(batch_size)
        ]
        ...
        targ = jnp.reshape(jnp.concatenate(targ_tensors, 1), targ_batch_shape)
        mask = jnp.transpose(
            jnp.reshape(
