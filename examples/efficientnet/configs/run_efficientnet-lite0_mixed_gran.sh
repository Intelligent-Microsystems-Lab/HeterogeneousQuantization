...
do
  ...
  python3 train.py --workdir=../../efficientnet-lite0_mixed_${SIZE}_gran_7 --config=efficientnet/configs/efficientnet-lite0_mixed_granular.py  --config.quant_target.weight_mb=${WEIGHT_TARGET} --config.quant_target.act_mb=${ACT_TARGET} --config.quant.bits=${BITS} --config.pretrained_quant=gs://imagenet_clemens/pretrained_hq/enet_${BITS}_pre_3/best
