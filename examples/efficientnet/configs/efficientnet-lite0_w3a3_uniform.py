...
from quant import uniform_static, percentile_init, gaussian_init, round_uniform_noise
...
def get_config():
  ...
  config.quant.stem.weight = partial(
      uniform_static, init_fn=partial(gaussian_init), round_fn=round_uniform_noise)
  ...
  config.quant.mbconv.act = partial(uniform_static, act=True, init_fn=partial(
      percentile_init, perc=99.9), round_fn=round_uniform_noise)
