...
def get_config():
  ...
  config.quant.stem.weight = partial(
      parametric_d_xmax)
  config.quant.stem.act = partial(parametric_d_xmax)

  # Conv in MBConv blocks.
  config.quant.mbconv = ml_collections.ConfigDict()
  config.quant.mbconv.weight = partial(
      parametric_d_xmax)
  config.quant.mbconv.act = partial(parametric_d_xmax)

  # Conv for head layer.
  config.quant.head = ml_collections.ConfigDict()
  config.quant.head.weight = partial(
      parametric_d_xmax)
  config.quant.head.act = partial(parametric_d_xmax)

  # Average quant.
  config.quant.average = partial(parametric_d_xmax)

  # Final linear layer.
  config.quant.dense = ml_collections.ConfigDict()
  config.quant.dense.weight = partial(
      parametric_d)
  config.quant.dense.act = partial(parametric_d_xmax)
