...
from quant import parametric_d_xmax, gaussian_init, percentile_init, round_invtanh, round_ewgs
...
def get_config():
  ...
  config.quant.stem.weight = partial(
      parametric_d_xmax, init_fn=partial(gaussian_init), round_fn=round_invtanh, bitwidth_min=1)
  ...
  config.quant.mbconv.act = partial(parametric_d_xmax, act=True, init_fn=partial(
      percentile_init, perc=99.9), round_fn=round_ewgs, bitwidth_min=1, d_max=8)
