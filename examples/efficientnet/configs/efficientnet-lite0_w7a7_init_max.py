...
def get_config():
  ...
  config.quant.stem.weight = partial(
      uniform_dynamic, bits=7, init_fn=max_init)
  config.quant.stem.act = partial(uniform_dynamic, bits=7, init_fn=max_init)

  # Conv in MBConv blocks.
  config.quant.mbconv = ml_collections.ConfigDict()
  config.quant.mbconv.weight = partial(
      uniform_dynamic, bits=7, init_fn=max_init)
  config.quant.mbconv.act = partial(uniform_dynamic, bits=7, init_fn=max_init)

  # Conv for head layer.
  config.quant.head = ml_collections.ConfigDict()
  config.quant.head.weight = partial(
      uniform_dynamic, bits=7, init_fn=max_init)
  config.quant.head.act = partial(uniform_dynamic, bits=7, init_fn=max_init)

  # Average quant.
  config.quant.average = partial(uniform_dynamic, bits=7, init_fn=max_init)

  # Final linear layer.
  config.quant.dense = ml_collections.ConfigDict()
  config.quant.dense.weight = partial(
      uniform_dynamic, bits=7, init_fn=max_init)
  config.quant.dense.act = partial(uniform_dynamic, bits=7, init_fn=max_init)
  config.quant.dense.bias = partial(uniform_dynamic, bits=7, init_fn=max_init)
