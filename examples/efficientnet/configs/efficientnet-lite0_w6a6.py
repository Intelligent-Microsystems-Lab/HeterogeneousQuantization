...
def get_config():
  ...
  config.quant.stem.act = partial(signed_uniform_max_scale_quant_ste, bits=6)

  # Conv in MBConv blocks.
  config.quant.mbconv = ml_collections.ConfigDict()
  config.quant.mbconv.weight = partial(
      signed_uniform_max_scale_quant_ste, bits=6)
  config.quant.mbconv.act = partial(signed_uniform_max_scale_quant_ste, bits=6)

  # Conv for head layer.
  config.quant.head = ml_collections.ConfigDict()
  config.quant.head.weight = partial(
      signed_uniform_max_scale_quant_ste, bits=6)
  config.quant.head.act = partial(signed_uniform_max_scale_quant_ste, bits=6)

  # Average quant.
  config.quant.average = partial(signed_uniform_max_scale_quant_ste, bits=6)
