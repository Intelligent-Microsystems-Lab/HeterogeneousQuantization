...
def get_config():
  ...
  config.quant.stem.weight = partial(
      parametric_d)
  config.quant.stem.act = partial(parametric_d)

  # Conv in MBConv blocks.
  config.quant.mbconv = ml_collections.ConfigDict()
  config.quant.mbconv.weight = partial(
      parametric_d)
  config.quant.mbconv.act = partial(parametric_d)

  # Conv for head layer.
  config.quant.head = ml_collections.ConfigDict()
  config.quant.head.weight = partial(
      parametric_d)
  config.quant.head.act = partial(parametric_d)

  # Average quant.
  config.quant.average = partial(parametric_d)

  # Final linear layer.
  config.quant.dense = ml_collections.ConfigDict()
  config.quant.dense.weight = partial(
      parametric_d)
  config.quant.dense.act = partial(parametric_d)
  config.quant.dense.bias = partial(parametric_d)
