...
def get_config():
  ...
  config.quant.stem.act = partial(parametric_d, bits=8, act=True)

  # Conv in MBConv blocks.
  config.quant.mbconv = ml_collections.ConfigDict()
  config.quant.mbconv.weight = partial(parametric_d, bits=8)
  config.quant.mbconv.act = partial(parametric_d, bits=8, act=True)

  # Conv for head layer.
  config.quant.head = ml_collections.ConfigDict()
  config.quant.head.weight = partial(parametric_d, bits=8)
  config.quant.head.act = partial(parametric_d, bits=8, act=True)

  # Average quant.
  config.quant.average = partial(parametric_d, bits=8, act=True)

  # Final linear layer.
  config.quant.dense = ml_collections.ConfigDict()
  config.quant.dense.weight = partial(parametric_d, bits=8)
  config.quant.dense.act = partial(parametric_d, bits=8, act=True)
