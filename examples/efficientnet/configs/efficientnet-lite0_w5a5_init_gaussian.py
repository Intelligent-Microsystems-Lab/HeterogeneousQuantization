...
def get_config():
  ...
  config.quant.stem.weight = partial(
      signed_uniform_max_scale_quant_ste, bits=5)
  config.quant.stem.act = partial(signed_uniform_max_scale_quant_ste, bits=5)

  # Conv in MBConv blocks.
  config.quant.mbconv = ml_collections.ConfigDict()
  config.quant.mbconv.weight = partial(
      signed_uniform_max_scale_quant_ste, bits=5)
  config.quant.mbconv.act = partial(signed_uniform_max_scale_quant_ste, bits=5)

  # Conv for head layer.
  config.quant.head = ml_collections.ConfigDict()
  config.quant.head.weight = partial(
      signed_uniform_max_scale_quant_ste, bits=5)
  config.quant.head.act = partial(signed_uniform_max_scale_quant_ste, bits=5)

  # Average quant.
  config.quant.average = partial(signed_uniform_max_scale_quant_ste, bits=5)

  # Final linear layer.
  config.quant.dense = ml_collections.ConfigDict()
  config.quant.dense.weight = partial(
      signed_uniform_max_scale_quant_ste, bits=5)
  config.quant.dense.act = partial(signed_uniform_max_scale_quant_ste, bits=5)
  config.quant.dense.bias = partial(signed_uniform_max_scale_quant_ste, bits=5)
