...
if __name__ == "__main__":
  ...
  # plot_curves(pc_noise, "Predictive Coding Noise",
  #             'pc_noise', 'Noise', args.samples)
  # plot_curves(pc_bits, "Predictive Coding Quantization",
  #             'pc_bits', 'Bits', args.samples)
  # plot_curves(bp_noise, "Backpropagation Noise",
  #             'bp_noise', 'Noise', args.samples)
  # plot_curves(bp_bits, "Backpropagation Quantization",
  #             'bp_bits', 'Bits', args.samples)
  # plot_diff(bp_noise, pc_noise, "Differences Noise",
  #           'diff_noise', 'Noise', args.samples)
  # plot_diff(bp_bits, pc_bits, "Differences Quantization",
  #           'diff_bits', 'Bits', args.samples)

  mean_bp_in, std1, x1 = mean_std_eval_acc1(
      args.basedir + "/err_inpt_bits_bp", 1)
  mean_bp_w, std1, x1 = mean_std_eval_acc1(
      args.basedir + "/err_weight_bits_bp", 1)
  mean_pc_in, std1, x1 = mean_std_eval_acc1(
      args.basedir + "/err_inpt_bits_pc", 1)
  mean_pc_w, std1, x1 = mean_std_eval_acc1(
      args.basedir + "/err_weight_bits_pc", 1)
  ...
  # ax.plot(x1[:-5], mean_pc_in[:-5, 0, 0]*100,
  #         color='red', label='Error (PC)', linewidth=5)
  # ax.plot(x1[:-5], mean_bp_in[:-5, 0, 0]*100, color='red',
  #         ls='--', label='Error (BP)', linewidth=5)

  # ax.plot(x1[:-5], mean_pc_w[:-5, 0, 0]*100, color='green',
  #         label='Gradients (PC)', linewidth=5)
  # ax.plot(x1[:-5], mean_bp_w[:-5, 0, 0]*100, color='green',
  #         ls='--', label='Gradients (BP)', linewidth=5)

  ax.plot(x1[:-4], np.mean(mean_pc_in[:-4, :, 0], axis=1) * 100,
          color='red', label='Error (PC)', linewidth=5)
  ax.plot(x1[:-4], np.mean(mean_bp_in[:-4, :, 0], axis=1) * 100,
          color='red', ls='--', label='Error (BP)', linewidth=5)

  ax.plot(x1[:-4], np.mean(mean_pc_w[:-4, :, 0], axis=1) * 100,
          color='green', label='Gradients (PC)', linewidth=5)
  ax.plot(x1[:-4], np.mean(mean_bp_w[:-4, :, 0], axis=1) * 100,
          color='green', ls='--', label='Gradients (BP)', linewidth=5)
  ...
  plt.tight_layout()
  plt.savefig("figures/bit_sweep_errors.png")
  plt.close()
