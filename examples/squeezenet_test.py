...
from squeezenet.configs import squeezenet_v11_fp32 as default_lib
...
class SqueezeNetTest(absltest.TestCase):
  """Test cases for SqueeezNet v1 model definition."""

  def test_squeezenet_model(self):
    """Tests SqueezeNet v1.1 model definition and output (variables)."""
    ...
    self.assertEqual(np.sum(jax.tree_util.tree_leaves(jax.tree_map(
        lambda x: np.prod(x.shape), variables['params']))), 1235496)

    # variables params and batch stats
    self.assertLen(variables, 2)

    # SqueezeNetv1.1 model will create parameters for the following layers:
    #   stem conv = 1
    #   Fire = 8
    #   head conv = 1
    self.assertLen(variables['params'], 10)

  def test_squeezenet_inference(self):

    # get correct config
    config_module = importlib.import_module(
        'configs.squeezenet_v11_fp32')
    ...
    state = squeezenet_load_pretrained_weights(state, config.pretrained)

    # load inpt
    input_image = Image.open('../../unit_tests/squeezenet_unit_test/dog.jpg')
    ...
    pytorch_stem = np.load('../../unit_tests/squeezenet_unit_test/stem.npy')
